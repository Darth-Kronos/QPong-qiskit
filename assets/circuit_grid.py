...
from . import circuit_grid_model
import qiskit
...
class CircuitGrid(pygame.sprite.RenderPlain):
    ...
    def __init__(self, x, y, circuit_grid_model, game_mode=2):
        self.x = x
        self.y = y
        self.circuit_grid_model = circuit_grid_model
        self.selected_wire = 0
        self.selected_column = 0
        self.circuit_grid_background = CircuitGridBackground(
            circuit_grid_model, game_mode
        )
    ...
    def update(self, *args):
        ...
        self.circuit_grid_background.rect.left = self.x
        self.circuit_grid_background.rect.top = self.y

        for row_idx in range(self.circuit_grid_model.max_wires):
            for col_idx in range(self.circuit_grid_model.max_columns):
                self.gate_tiles[row_idx][col_idx].rect.centerx = self.x + GRID_WIDTH * (
                    col_idx + 1.5
                )
                self.gate_tiles[row_idx][
                    col_idx
                ].rect.centery = self.y + GRID_HEIGHT * (row_idx + 1.0)
    ...
    def highlight_selected_node(self, wire_num, column_num):
        ...
        self.circuit_grid_cursor.rect.left = (
            self.x
            + GRID_WIDTH * (self.selected_column + 1.0)
    ...
    def handle_input_x(self):
        ...
        # circuit_grid_node = circuit_grid_model.CircuitGridNode(node_types.X)
    ...
    def handle_input_y(self):
        selected_node_gate_part = self.get_selected_node_gate_part()
        if selected_node_gate_part == node_types.EMPTY:
            circuit_grid_node = circuit_grid_model.CircuitGridNode(node_types.Y)
            self.circuit_grid_model.set_node(
                self.selected_wire, self.selected_column, circuit_grid_node
            )
    ...
    def handle_input_z(self):
        selected_node_gate_part = self.get_selected_node_gate_part()
        if selected_node_gate_part == node_types.EMPTY:
            circuit_grid_node = circuit_grid_model.CircuitGridNode(node_types.Z)
            self.circuit_grid_model.set_node(
                self.selected_wire, self.selected_column, circuit_grid_node
            )
    ...
    def handle_input_h(self):
        selected_node_gate_part = self.get_selected_node_gate_part()
        if selected_node_gate_part == node_types.EMPTY:
            circuit_grid_node = circuit_grid_model.CircuitGridNode(node_types.H)
            self.circuit_grid_model.set_node(
                self.selected_wire, self.selected_column, circuit_grid_node
            )
    ...
    def handle_input_delete(self):
        ...
        if selected_node_gate_part == node_types.CTRL:
            ...
            circuit_grid_node = circuit_grid_model.CircuitGridNode(node_types.EMPTY)
            self.circuit_grid_model.set_node(
                self.selected_wire, self.selected_column, circuit_grid_node
            )
                ...
                for wire_num in range(
                    ...
                    if (
                        ...
                        self.circuit_grid_model.set_node(
                            ...
                            circuit_grid_model.CircuitGridNode(node_types.EMPTY),
                        )
                self.update()
            else:
                        ...
                        if direction == MOVE_UP and candidate_wire_num < self.selected_wire:
                            if self.model.get_node_gate_part(candidate_wire_num + 1,
                                ...
                                self.model.set_node(candidate_wire_num + 1, self.selected_column,
                                                             circuit_grid_model.CircuitGridNode(node_types.TRACE))
    ...
    def place_ctrl_qubit(self, gate_wire_num, candidate_ctrl_wire_num):
        ...
        if (
            ...
            or candidate_ctrl_wire_num >= self.circuit_grid_model.max_wires
        ):
            return -1
...
class CircuitGridBackground(pygame.sprite.Sprite):
    ...
    def __init__(self, circuit_grid_model, game_mode=2):
        pygame.sprite.Sprite.__init__(self)

        self._background_width = (
            GRID_WIDTH // 2 if game_mode == 2 else GRID_WIDTH
        )

        self.image = pygame.Surface(
            [self._background_width * (18 + 2), GRID_HEIGHT * (3 + 1)]
        )
        self.image.convert()
...
class CircuitGridGate(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if node_type == node_types.H:
            self.image, self.rect = resources.resources.load_image('gates/h_gate.png', -1)
            ...
            if node.ctrl_a >= 0 or node.ctrl_b >= 0:
                if self.wire_num > max(node.ctrl_a, node.ctrl_b):
                    self.image, self.rect = resources.resources.load_image('gates/not_gate_below_ctrl.png', -1)
                else:
                    self.image, self.rect = resources.resources.load_image('gates/not_gate_above_ctrl.png', -1)
            elif node.radians != 0:
                self.image, self.rect = resources.resources.load_image('gates/rx_gate.png', -1)
                self.rect = self.image.get_rect()
                ...
                self.image, self.rect = resources.resources.load_image('gates/x_gate.png', -1)
        elif node_type == node_types.Y:
            node = self.circuit_grid_model.get_node(self.wire_num, self.column_num)
            if node.radians != 0:
                self.image, self.rect = resources.resources.load_image('gates/ry_gate.png', -1)
                self.rect = self.image.get_rect()
                pygame.draw.arc(self.image, globals.MAGENTA, self.rect, 0, node.radians % (2 * np.pi), 6)
                pygame.draw.arc(self.image, globals.MAGENTA, self.rect, node.radians % (2 * np.pi), 2 * np.pi, 1)
            else:
                self.image, self.rect = resources.resources.load_image('gates/y_gate.png', -1)
        elif node_type == node_types.Z:
            node = self.circuit_grid_model.get_node(self.wire_num, self.column_num)
            if node.radians != 0:
                self.image, self.rect = resources.resources.load_image('gates/rz_gate.png', -1)
                self.rect = self.image.get_rect()
                pygame.draw.arc(self.image, globals.MAGENTA, self.rect, 0, node.radians % (2 * np.pi), 6)
                pygame.draw.arc(self.image, globals.MAGENTA, self.rect, node.radians % (2 * np.pi), 2 * np.pi, 1)
            else:
                self.image, self.rect = resources.resources.load_image('gates/z_gate.png', -1)
        elif node_type == node_types.S:
            self.image, self.rect = resources.resources.load_image('gates/s_gate.png', -1)
        elif node_type == node_types.SDG:
            self.image, self.rect = resources.resources.load_image('gates/sdg_gate.png', -1)
        elif node_type == node_types.T:
            self.image, self.rect = resources.resources.load_image('gates/t_gate.png', -1)
        elif node_type == node_types.TDG:
            self.image, self.rect = resources.resources.load_image('gates/tdg_gate.png', -1)
        elif node_type == node_types.IDEN:
            self.image, self.rect = resources.resources.load_image('gates/iden_gate.png', -1)
        elif node_type == node_types.CTRL:
            if self.wire_num > \
                    self.circuit_grid_model.get_gate_wire_for_control_node(self.wire_num, self.column_num):
                self.image, self.rect = resources.resources.load_image('gates/ctrl_gate_bottom_wire.png', -1)
            else:
                self.image, self.rect = resources.resources.load_image('gates/ctrl_gate_top_wire.png', -1)
        elif node_type == node_types.TRACE:
            self.image, self.rect = resources.resources.load_image('gates/trace_gate.png', -1)
        elif node_type == node_types.SWAP:
            self.image, self.rect = resources.resources.load_image('gates/swap_gate.png', -1)
        else:
